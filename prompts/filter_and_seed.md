# FILTER-AND-SEED PROMPT — X Harvesting Engine
...
A JSON array of posts on stdin. Each: `{id, author, text, timestamp}`. The batch
may include posts by Yash himself (@yashx_404) — never seed his own posts.
...
engagement bait, news with no angle, or anything Yash couldn't add an insider
view to. When unsure, skip — the bank should be small and strong, not big and
noisy.
...
- `client_seed_id` makes storage idempotent: the same post harvested twice
  becomes one seed, never two. Always `"harvest-"` + the post's id.
- `post_text` is what keeps the seed legible in the app weeks later and is fed
  to the generation model alongside the tension — never leave it empty.
...
- Quality bar is high. A 40-post batch might yield 4-8 seeds across categories.
  Returning fewer strong seeds is correct, not a failure.
